...
cp15::reset ()
{
  ...
  SCC_RB[SEC_NONSEC_VEC_BASE].permissions[arm_impl::PL1] =
    (CL_READ | CL_WRITE);
  ...
  if (crn == 0)
      ...
      if (opc1 == 0)
	{
	  ...
	  if (crm == 1)
	    {
	      if (opc2 == 4)
		return &(SCC_RB[MEM_MODEL_FEAT_0]);
	    }
	  if (crm == 2)
	    {
	      if (opc2 == 0)
		return &(SCC_RB[IS_ATTR_0]);
	      if (opc2 == 1)
		return &(SCC_RB[IS_ATTR_1]);
	      if (opc2 == 2)
		return &(SCC_RB[IS_ATTR_2]);
	      if (opc2 == 3)
		return &(SCC_RB[IS_ATTR_3]);
	      if (opc2 == 4)
		return &(SCC_RB[IS_ATTR_4]);
	      if (opc2 == 5)
		return &(SCC_RB[IS_ATTR_5]);
	      if (opc2 == 6)
		return &(SCC_RB[IS_ATTR_6]);
	      if (opc2 == 7)
		return &(SCC_RB[IS_ATTR_7]);
	    }
	}
      if (opc1 == 1)
	{
  ...
  if (crn == 1)
      ...
      if (opc1 == 0)
	{
	  if (crm == 0)
		...
		return &(SCC_RB[CTR]);
	      if (opc2 == 1)
		return &(SCC_RB[AUX_CTR]);
	      if (opc2 == 2)
		return &(SCC_RB[COPROC_ACC_CTR]);
	    }
	  if (crm == 1)
	    {
	      if (opc2 == 0)
		return &(SCC_RB[SEC_CONF]);
	      if (opc2 == 1)
		return &(SCC_RB[SEC_DBG_ENABLE]);
	      if (opc2 == 2)
		return &(SCC_RB[NONSEC_ACC_CTR]);
	    }
	}
    }
  if (crn == 2)
  ...
  if (crn == 5)
      ...
      if (opc1 == 0)
	{
	  ...
	  if (crm == 1)
	    {
	      if (opc2 == 0)
		return &(MMU_RB[DFSR_AUX]);
	      if (opc2 == 1)
		return &(MMU_RB[IFSR_AUX]);
	    }
	}
    }
  if (crn == 6)
  ...
  if (crn == 10)
      ...
      if (opc1 == 0)
	{
	  if (crm == 2)
	    {
	      if (opc2 == 0)
		return &(MMU_RB[PRIMARY_REGION_REMAP]);
	      if (opc2 == 1)
		return &(MMU_RB[NORMAL_REGION_REMAP]);
	    }
	}
    }
  if (crn == 12)
      ...
      if (opc1 == 0)
	{
	  if (crm == 0)
	    {
	      if (opc2 == 0)
		return &(SCC_RB[SEC_NONSEC_VEC_BASE]);
	      if (opc2 == 1)
		return &(SCC_RB[MON_VEC_BASE_ADD]);
	    }
	}
    }
  if (crn == 13)
      ...
      if (opc1 == 0)
	{
	  if (crm == 0)
	    {
	      if (opc2 == 0)
		return &(MMU_RB[FCSE_PID]);
	      if (opc2 == 1)
		return &(MMU_RB[CONTEXT_ID]);
	      if (opc2 == 2)
		return &(MMU_RB[TPID_RW]);
	      if (opc2 == 3)
		return &(MMU_RB[TPID_RO]);
	      if (opc2 == 4)
		return &(MMU_RB[TPID_PO]);
	    }
	}
    }
  return NULL;
	   ...
	   const unsigned crm, const unsigned rt_value)
{
  ...
  if (!(dest->permissions[pl] & CL_WRITE))
    {
	   ...
	   const unsigned crn, const unsigned crm)
{
  ...
  if (!(dest->permissions[pl] & CL_READ))
    {
...
void
cp15::TLB_operations (const unsigned opc1, const unsigned opc2,
		      const unsigned crn, const unsigned crm,
