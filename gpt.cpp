...
void gpt_module::set_output_pin(bool &output, oc_operation_mode_t mode) {
  switch(mode) {
  case OC_TOGGLE:
    output = !output;
    break;
...
unsigned gpt_module::fast_read(unsigned address) {
  unsigned res = 0;
  switch(address) {
  case GPT_CNT:
    return counter;
...
void gpt_module::fast_write(unsigned address, unsigned datum) {
  switch(address) {
  case GPT_CR:
    // Not written to the control register
    if (datum & (1 << 31)) { // force output compare channel 3
      set_output_pin(do_cmpout3, om3);
    }
    if (datum & (1 << 30)) { // force output compare channel 2
      set_output_pin(do_cmpout2, om2);
    }
    if (datum & (1 << 29)) { // force output compare channel 1
      set_output_pin(do_cmpout1, om1);
    }
    ...
    counter_mode = static_cast<counter_mode_t>((datum & 0x00000200) >> 9);
    clock_src = static_cast<selected_clock_t>((datum & 0x000001C0) >> 6);
...
void gpt_module::ind_capin1(bool deassert) {
  if (((im1 == IC_RISINGEDGE || im1 == IC_BOTH) 
       && !deassert)
      || ((im1 == IC_FALLINGEDGE || im1 == IC_BOTH) 
          && deassert)) {
...
void gpt_module::ind_capin2(bool deassert) {
  if (((im2 == IC_RISINGEDGE || im2 == IC_BOTH) 
       && !deassert)
      || ((im2 == IC_FALLINGEDGE || im2 == IC_BOTH) 
          && deassert)) {
