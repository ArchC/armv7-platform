...
void tzic_module::prc_tzic() {
  do {    
    ...
    for (unsigned i = 0; i < 128; ++i) {
      ...
      switch (i % 4) {
      case 0: pri = pri & 0xFF; break;
      case 1: pri = (pri & 0xFF00) >> 8; break;
      case 2: pri = (pri & 0xFF0000) >> 16; break;
      case 3: pri = (pri & 0xFF000000) >> 24; break;
      }
...
void tzic_module::interrupt(unsigned intnumber, bool deassert) {
...
unsigned tzic_module::fast_read(unsigned address) {
  unsigned res = 0;
  switch (address) {
  case TZIC_INTCTRL:
    if (enabled)
...
void tzic_module::fast_write(unsigned address, unsigned datum) {
  changed = true;
  switch (address) {
  case TZIC_INTCTRL:
    if (datum & 1)
