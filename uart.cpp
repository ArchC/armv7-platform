...
static void set_raw_terminal_mode()
{
...
uart_module::uart_module (sc_module_name name_, tzic_module &tzic_): sc_module(name_),
                                                                     tzic(tzic_) 
{
...
static bool check_if_has_input() {
...
unsigned uart_module::fast_read(unsigned address) {
  unsigned res = 0;
  switch(address) {
    // Reading character
  case UART_URXD: {
...
void uart_module::fast_write(unsigned address, unsigned datum) {
  switch(address) {
  case UART_UTXD: {
