// gpt.h
// -
// This represents the General Purpose Timer used in the ARM SoC by 
// Freescale iMX35.
//
...
class gpt_module : public sc_module, public peripheral {
 private:
  ...
  enum selected_clock_t {CLK_OFF=0, PERIPHERAL_CLK=1, HI_FREQ=2, EXTERNAL_CLK=3,
                         LOW_FREQ=4};

  enum oc_operation_mode_t {OC_DISCONNECTED=0, OC_TOGGLE=1, OC_CLEAR=2,
                            OC_SET=3, OC_LOWPULSE=4};

  enum input_capture_t {IC_DISABLED=0, IC_RISINGEDGE=1, IC_FALLINGEDGE=2,
                        IC_BOTH=3};

  enum counter_mode_t {M_RESTART=0, M_FREERUN=1};
